...
def resolve_trade_targets(
    *,
    ...
    Used by both backtesting._enter_ranked_candidates and the live entry path
    so both paths always produce identical exit levels for the same trade inputs.
    """
    ...
    asset_class = asset_class_map.get(str(engine_name), "INTRADAY_EQUITY")

    targets = calculate_cost_aware_targets(
        entry_price=float(entry_price),
        quantity=int(quantity),
        asset_class=asset_class,
        risk_profile=str(risk_style_name),
        signal_strength=float(signal_strength),
        side=str(side),
    )

    trailing_distance = float(atr) * float(trailing_atr_multiplier)
    stop_distance = abs(float(entry_price) - float(targets["stop_loss"]))

    if str(engine_name) == "delivery_equity":
        trailing_activation_distance = engine_helper.get_trailing_activation_distance(
            float(entry_price),
            float(targets["target"]),
            float(atr),
        )
    else:
