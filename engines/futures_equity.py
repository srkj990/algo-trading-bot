...
class FuturesEquityEngine(TradingEngine):
    ...
    def get_cycle_state(self, now):
        ...
        if current_time < self.market_open or current_time >= self.market_close:
            return {
                ...
                "force_square_off": False,
                "allow_scan": False,
                "reason": "Market closed for futures trading",
            }
    ...
    def reconcile_startup(self, execution_mode, persisted_positions):
        ...
        try:
            broker_positions = {}
            for item in get_futures_positions():
            ...
            log_event(
                f"[RECON] Loaded {len(broker_positions)} live futures holdings from broker"
            )
            return broker_positions
        except NotImplementedError as ex:
            log_event(f"[RECON] Futures startup sync unavailable: {ex}", "warning")
            return persisted_positions
